  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((dept) => (
            <div key={dept._id} className="bg-gray-900 border border-gray-800 rounded-2xl p-5 hover:border-gray-700 transition-all group">
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-3">
                  <div>
                    <h3 className="font-semibold text-white text-sm">{dept.name}</h3>
                    <span className={`text-xs px-2 py-0.5 rounded-full ${dept.status === "Active" ? "bg-emerald-500/20 text-emerald-400" : "bg-red-500/20 text-red-400"}`}>
                      {dept.status}
                    </span>
                  </div>
                </div>
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-all">
                  <button onClick={() => openEdit(dept)} className="p-1.5 rounded-lg text-blue-400 hover:bg-blue-500/10 transition-all">
                    <FiEdit2 className="text-xs" />
                  </button>
                  <button onClick={() => handleDelete(dept._id)} className="p-1.5 rounded-lg text-red-400 hover:bg-red-500/10 transition-all">
                    <FiTrash2 className="text-xs" />
                  </button>
                </div>
              </div>
              <p className="text-gray-500 text-xs line-clamp-2">{dept.description || "No description."}</p>
              {dept.location && <p className="text-gray-600 text-xs mt-2">📍 {dept.location}</p>}
            </div>
          ))}
        </div>
      )}
