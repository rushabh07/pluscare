    ...
    <div className="space-y-6">
      ...
      {loading ? (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {[...Array(4)].map((_, i) => <div key={i} className="h-64 bg-white border border-slate-200 rounded-2xl animate-pulse" />)}
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-5">
            <h2 className="text-sm font-semibold text-slate-900 mb-5 flex items-center gap-2">
              <FiTrendingUp className="text-blue-600" /> Monthly Appointments (Last 6 Months)
            </h2>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-5">
            <h2 className="text-sm font-semibold text-slate-900 mb-5 flex items-center gap-2">
              <FiActivity className="text-purple-600" /> Appointments by Department
            </h2>
            {depts.length === 0 ? (
              <p className="text-slate-500 text-sm text-center py-10">No department data yet.</p>
            ) : (
              <div className="space-y-3">
                {depts.map(([dept, count], i) => (
                  <div key={dept}>
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-sm text-slate-600 truncate max-w-[160px]">{dept}</span>
                      <span className="text-sm text-slate-900 font-medium">{count}</span>
                    </div>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-5">
            <h2 className="text-sm font-semibold text-slate-900 mb-5 flex items-center gap-2">
              <FiUsers className="text-cyan-700" /> User Role Distribution
            </h2>
              ...
              {[
                ...
                <div key={r.label}>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="text-sm text-slate-900 font-medium">{r.count} ({Math.round((r.count / total) * 100)}%)</span>
                  </div>
            ...
            <p className="text-xs text-slate-500 mt-4">Total: {data.users.length} users</p>
          </div>

          {/* Revenue Analytics */}
          <div className="bg-white border border-slate-200 rounded-2xl p-5">
            <h2 className="text-sm font-semibold text-slate-900 mb-5 flex items-center gap-2">
              <FiPieChart className="text-emerald-700" /> Revenue Analytics
            </h2>
            <div className="grid grid-cols-2 gap-4 mb-4">
              <div className="bg-slate-100/50 rounded-xl p-4">
                <p className="text-slate-500 text-xs">Total Collected</p>
                <p className="text-xl font-bold text-emerald-700 mt-1">₹{totalRevenue.toLocaleString()}</p>
              </div>
              <div className="bg-slate-100/50 rounded-xl p-4">
                <p className="text-slate-500 text-xs">Avg Bill Value</p>
                <p className="text-xl font-bold text-slate-900 mt-1">₹{Number(avgPerBill).toLocaleString()}</p>
              </div>
              <div className="bg-slate-100/50 rounded-xl p-4">
                <p className="text-slate-500 text-xs">Total Bills</p>
                <p className="text-xl font-bold text-slate-900 mt-1">{data.billing.length}</p>
              </div>
              <div className="bg-slate-100/50 rounded-xl p-4">
                <p className="text-slate-500 text-xs">Pending Bills</p>
